import {
  useCreateLoungeMutation,
  useUpdateLoungeMutation,
} from "../../../services/hooks/Lounge/useLoungeMutation";
import { useDeleteImageMutation } from "../../../services/hooks/images/useImagesMutation";
import { TextInput, Button, Text, ActionIcon } from "@mantine/core";
import { useField } from "@mantine/form";
import { useCallback, useMemo, useRef, useState } from "react";
import { toast } from "sonner";
import { Building, Camera, MapPin } from "lucide-react";
import { Avatar } from "@mui/material";
import { uploadImage } from "../../../utils/uploadImage";
import { useAccountStore } from "../../../store/useAccountStore";

export const LoungeForm = ({ lounge, onClose }) => {
  const fileInputRef = useRef(null);
  const { accessToken } = useAccountStore();
  const createLounge = useCreateLoungeMutation();
  const updateLounge = useUpdateLoungeMutation();
  ...
  const srcImg = useMemo(() => {
    ...
    if (lounge) {
      if (lounge?.image_url) {
        return lounge.image_url;
      }
    }
    return "./placeholder-lounge.webp";
  }, [localImage, lounge]);
    ...
    if (lounge) {
      updateLounge.mutate(
        { id: lounge.id, data },
        {
          onSettled: () => {
            if (lounge.image_url) {
              const imageName = getFileNameFromUrl(lounge.image_url);
      ...
      <div className="flex w-11/12 flex-col gap-5 rounded-lg">
        ...
        <div className="mt-10 w-full flex justify-end gap-7">
          ...
          <Button
            ...
            color="blue"
            variant="light"
            loading={createLounge.isPending || updateLounge.isPending}
            disabled={disabled}
            onClick={() => onSubmit()}
          >
            {lounge ? "Guardar" : "Registrar"}
          </Button>
        </div>
      </div>
    </div>
